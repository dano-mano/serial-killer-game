...
The NPC witness system is the primary data source for the evidence system (piece 10). Every `WitnessEvent` logged on an NPC becomes a potential clue the fed can uncover by interviewing that NPC. The reliability field on `WitnessStatement` is essential for the fed's gameplay loop — unreliable witnesses add genuine uncertainty, forcing the fed to cross-reference multiple sources.
